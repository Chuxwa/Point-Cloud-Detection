...
def parse_config():
    ...
    parser.add_argument("--tcp_port", type=int, default=18886, help="tcp port for distrbuted training")
...
def main():
    ...
    if args.ckpt is not None:
        ...
        if len(ckpt_list) > 0:
            ...
            it, start_epoch = model.load_params_with_optimizer(ckpt_list[-1], to_cpu=dist, optimizer=optimizer, logger=logger)
            last_epoch = start_epoch + 1
    ...
    lr_scheduler, lr_warmup_scheduler = build_scheduler(
        optimizer, total_iters_each_epoch=len(train_loader), total_epochs=args.epochs, last_epoch=last_epoch, optim_cfg=cfg.OPTIMIZATION
    )
